...
const ProductCard = ({ product, index, isFeatured }: { product: Product, index?: number, isFeatured?: boolean }) => {
  ...
  return (
    <Link to={`/product/${product.id}`} className="group block w-full select-none">
      <div className={`relative bg-white overflow-hidden rounded-[32px] border border-black/[0.05] shadow-[0_2px_20px_rgba(0,0,0,0.04)] flex items-center justify-center ${isFeatured ? 'w-full h-auto aspect-auto' : 'aspect-[4/5]'}`}>
        <img 
          src={product.image} 
          alt={product.name} 
