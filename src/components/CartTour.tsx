...
export const CartTour = ({ onStepChange }: CartTourProps = {}) => {
  ...
  useEffect(() => {
    ...
    const hasSeenTour = localStorage.getItem("cart_tour_completed");
  ...
  const completeTour = () => {
    localStorage.setItem("cart_tour_completed", "true");
    setIsVisible(false);
  };
  ...
  if (targetRect) {
    ...
    // For Desktop logic on Cart
    if (isDesktop && currentStep === 0) { // Cart Items
      if (spaceRight >= tooltipWidth + 24) {
        placement = 'right';
      }
    } else if (isDesktop && currentStep === 2) { // Checkout
